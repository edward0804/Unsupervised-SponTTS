...
# pred : prediction
class Trainer:
    def __init__(self, paths, restore_step: int = 0, unk_cls: int = 100):
        # === Init === #
        self.paths = paths
        ...
        (
            ...
            self.train_syns_loader,
        ) = self.__init_dataset()
        self.train_logger, self.eval_logger = self.__init_logger()
        self.vocoder = Vocoder("melgan")

        # === nn === #
        self.model = self.__init_model(n_spkers=self.n_spkers, unk_cls=unk_cls).to(self.device)
    ...
    def train(self):
        ...
        while self.train_step < hp.total_steps:
            # print('loading data...')
            for batches in tqdm(self.train_loader):
                for batch in batches:
                    ...
                    # === 3. Cal Loss ===#
                    mel_loss, mel_postnet_loss, d_loss, f_loss, e_loss, unk_loss, unk_place_loss = self.loss(
                        *model_pred, *gt_batch, ~text_mask, ~mel_mask,
                    )
                    total_loss = mel_loss + mel_postnet_loss + d_loss + f_loss + e_loss + unk_loss + unk_place_loss
    ...
    def __init_dataset(self):
        train_dataset = Dataset(data_dir=self.paths["data_dir"], split="train", unk_cls=self.unk_cls)
        # print(self.paths["data_dir"])
        valid_dataset = Dataset(data_dir=self.paths["data_dir"], split="valid", unk_cls=self.unk_cls)
        ...
        train_syns_loader = DataLoader(
            torch.utils.data.Subset(train_dataset, torch.tensor([0])),
            batch_size=hp.batch_size ** 2,
            shuffle=False,
            collate_fn=train_dataset.collate_fn,
            drop_last=False,
        ...
        return train_loader, valid_loader, n_spkers, spker_table, inv_spker_table, train_syns_loader

    def __init_model(self, n_spkers, unk_cls):
        model = nn.DataParallel(FastSpeech2(n_spkers=n_spkers, unk_cls=unk_cls))
    ...
    def __log(
        self, mode, total_loss, mel_loss, mel_postnet_loss, d_loss, f_loss, e_loss, unk_loss, unk_place_loss
    ):
        ...
        f_loss = f_loss.item()
        e_loss = e_loss.item()
        unk_loss= unk_loss.item()
        unk_place_loss = unk_place_loss.item()
        ...
        utils.simple_table(
            [
                ...
                ("Unk Loss", f"{unk_loss:.4f}"),
                ("Unk Place Loss", f"{unk_place_loss:.4f}"),
            ]
        )
    ...
    def __eval_and_synth(self):
        ...
        # total_loss, mel_loss, mel_postnet_loss, d_loss, f_loss, e_loss, unk_loss, unk_place_loss
        L = (torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0))
        for batches in tqdm(self.valid_loader):
            for batch in batches:
                # model_batch, gt_batch = utils.data_to_device(batch, self.device, mode='eval')
                model_batch, gt_batch = utils.data_to_device(batch, self.device, mode='train')
                with torch.no_grad():
                    # forward
                    (model_pred, text_mask, mel_mask, mel_lens) = self.model(
                        *model_batch
                    ...
                    # l = (total_loss, mel_loss, mel_postnet_loss, d_loss, f_loss, e_loss, unk_loss, unk_place_loss)
                    # print(l)
                    # accumulate loss
    ...
    def __train_synth(self):
        self.model.eval()
        print(f"[Evaluation] Evaluating training set at step {str(self.train_step)}")
        batch_num = 0
        gt_synth_path = self.paths["train_synth_path"] / "gt"
        pred_synth_path = self.paths["train_synth_path"] / str(self.train_step)

        synth_gt = True if not gt_synth_path.exists() else False
        # total_loss, mel_loss, mel_postnet_loss, d_loss, f_loss, e_loss, unk_loss, unk_place_loss
        L = (torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0))
        for batches in tqdm(self.train_syns_loader):
            for batch in batches:
                # model_batch, gt_batch = utils.data_to_device(batch, self.device, mode='eval')
                model_batch, gt_batch = utils.data_to_device(batch, self.device, mode='train')
                with torch.no_grad():
...
def main(args):
    ...
    paths["train_synth_path"] = Path(record_root / record_idx / "train_synth").resolve()
    ...
    # === training === #
    torch.manual_seed(0)
    trainer = Trainer(paths, restore_step=args.restore_step, unk_cls=args.unk_cls)
    trainer.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--unk_cls", type=int, default=100)

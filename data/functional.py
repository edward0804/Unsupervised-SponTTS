...
def process_utterance(
    ...
    * You can set some constrains by returning None.
    ...
    if not textgrid_path.exists() or not wav_path.exists():
        print(basename, 'text or wav not exists')
        return None
    ...
    # if os.path.exists(ali_path) and os.path.exists(f0_path) and os.path.exists(energy_path) and os.path.exists(mel_path) and os.path.exists(text_path):
    #   return {}
    ...
    wav_len = librosa.get_duration(wav, sr=sample_rate)
    ...
    # print(total_length / sample_rate * hp.hop_length , trim_end - trim_start)
    assert(abs(total_length / sample_rate * hp.hop_length - (trim_end - trim_start)) < 0.2)
